...
$user = Auth::user();
$role = $user['role'] ?? null;
$canManage = in_array($role, ['Admin'], true);

$error = '';
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    require_post_csrf();
    $action = $_POST['action'] ?? '';
    if ($canManage && $action === 'create_tx') {
  ...
  } elseif ($canManage && $action === 'confirm_tx') {
    $id = (int)($_POST['id'] ?? 0);
...
$students = $canManage ? $pdo->query('SELECT id, first_name, last_name FROM students ORDER BY first_name')->fetchAll() : [];
$txs = $canManage ? $pdo->query('SELECT t.*, CONCAT(s.first_name, " ", s.last_name) AS student FROM transactions t JOIN students s ON s.id = t.student_id ORDER BY t.id DESC')->fetchAll() : [];
$daily = $canManage ? $pdo->query("SELECT DATE(created_at) dt, COUNT(*) cnt, SUM(amount) sum FROM transactions GROUP BY DATE(created_at) ORDER BY dt DESC")->fetchAll() : [];
...
<?php else: ?>
  ...
  <ul>
    <li>Fawry (in-person code or online gateway)</li>
    <li>Vodafone Cash</li>
  </ul>
  <p style="color:#6b7280;">Contact administration to generate a payment reference, or visit the cashier if enabled.</p>
<?php endif; ?>
